-- Tradovate trades: repair numeric tickers + NULL pnl (one-time production backfill).
-- Fee columns are not stored on trades or executions; backfilled pnl matches importer with fees=0 (gross net).
-- Multipliers mirror lib/integrations/tradovate/futuresValuePerPointFallback.ts (ROOT_VALUE_PER_POINT).

-- =============================================================================
-- 1) DRY RUN — inspect candidates (no writes)
-- =============================================================================
...
with repair_candidates as (
  select
    ...
    t.contracts,
    t.pnl as old_pnl,
    nullif(trim(split_part(t.broker_lifecycle_id, ':', 4)), '') as lifecycle_contract_id,
    coalesce(
...
select
  trade_id,
  old_ticker,
  resolved_root as resolved_ticker,
  points,
  contracts,
  value_per_point,
  old_pnl,
  round((points * contracts * value_per_point)::numeric, 2) as calculated_pnl,
  broker_lifecycle_id
from with_vpp
...
-- =============================================================================
-- 2) UPDATE — run in a transaction after dry-run looks correct
-- =============================================================================

-- begin;

-- with repair_candidates as (
--   ... same CTEs as dry-run ...
-- ),
-- with_vpp as ( ... ),
-- final as (
--   select
...
-- select count(*) as rows_updated from final;  -- run as separate select before commit if needed

-- rollback;  -- remove and use commit; when satisfied
