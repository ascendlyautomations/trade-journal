-- Repair: rpc_v1_profile_bootstrap referenced nonexistent stories.expires_at.
-- Active stories use created_at within the last 24 hours (matches lib/activeStories.ts).
...
begin
  ...
  if v_can_view and v_tab = 'trades' then
    ...
    -- Set-based engagement for visible trade ids
    with trade_ids as (
...
comment on function public.rpc_v1_profile_bootstrap(text, text, integer, text) is
  'Bounded Profile header + active-tab first page (read-only).';
