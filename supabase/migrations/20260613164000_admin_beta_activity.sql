-- Admin Beta Dashboard: paginated activity feed (separate from stat bundle).
...
revoke all on function public.admin_beta_activity(int, int, text) from public;
grant execute on function public.admin_beta_activity(int, int, text) to authenticated;

comment on function public.admin_beta_activity(int, int, text) is
  'Admin-only paginated beta activity (bugs, features, beta room messages, beta trades).';
