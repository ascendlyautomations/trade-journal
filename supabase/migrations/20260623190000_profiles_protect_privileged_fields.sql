-- Block authenticated users from self-updating billing, moderation, and referral ledger fields.
-- Service role (Stripe webhooks, server routes) and admin_users may still update any column.
-- Beta referral grant (referred_by → TRAXBETA10302) may set is_pro / is_beta_tester via sibling trigger.
...
comment on function public.profiles_reject_privileged_self_update() is
  'Rejects self-updates to billing, ban, and referral ledger columns. Bypassed for service_role, admins, and beta referral grant.';

drop trigger if exists profiles_protect_privileged_fields_trigger on public.profiles;

create trigger profiles_protect_privileged_fields_trigger
  before update on public.profiles
  for each row
  execute function public.profiles_reject_privileged_self_update();

comment on trigger profiles_protect_privileged_fields_trigger on public.profiles is
  'Prevents privilege escalation via client profile updates; runs after profiles_beta_referral_trigger (name order).';
