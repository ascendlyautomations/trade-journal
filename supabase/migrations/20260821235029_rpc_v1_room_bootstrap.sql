-- Backend V2 Phase F: Trade Room bootstrap — single JSON for initial selected-room load.
-- SECURITY DEFINER with auth.uid() membership checks (same family as mark_room_read).
-- VOLATILE: may perform mark_room_read when p_mark_read = true.
...
begin
  ...
  -- Pinned messages (bounded, section-filtered)
  with filtered as (
    select msg.*
  ...
  -- Main messages (keyset page: created_at DESC, id DESC)
  with filtered as (
  ...
  return jsonb_build_object(
    ...
    'data', jsonb_build_object(
      'room', jsonb_build_object(
        ...
        'is_public', coalesce(v_room.is_public, true),
        'allow_members_chat', coalesce(v_room.allow_members_chat, true),
        'show_on_profile', coalesce(v_room.show_on_profile, true),
        'created_at', case
          when v_room.created_at is null then null
          else to_char(timezone('utc', v_room.created_at), 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
        end
      ),
...
-- Helper: one message row with profile, trade id, reactions (viewer-safe projection).
create or replace function public.rpc_v1_room_bootstrap_message_row(p_message_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    ...
    'room_id', m.room_id,
    'user_id', m.user_id,
    'seen_by', coalesce(m.seen_by, '[]'::jsonb),
    ...
    'trades', case
      ...
      else jsonb_build_object('id', m.trade_id)
    end,
    'profiles', (
      select jsonb_build_object(
...
comment on function public.rpc_v1_room_bootstrap is
  'Phase F: bounded Trade Room bootstrap — sections, prefs, messages, optional mark_read.';
