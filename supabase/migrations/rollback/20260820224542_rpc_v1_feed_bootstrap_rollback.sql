-- Phase B2 rollback: restore pre-optimization Feed RPC (timestamptz cursor).
-- Run manually on local/staging after validation cycle — NOT applied to production automatically.

drop function if exists public.rpc_v1_feed_bootstrap(text, text, integer, text);
drop function if exists public._v1_feed_before_cursor(
  timestamptz, text, uuid, timestamptz, text, uuid, integer, boolean
);
drop function if exists public._v1_feed_parse_cursor(text);
drop function if exists public._v1_feed_kind_rank(text);

drop index if exists public.profile_posts_user_id_created_at_idx;

-- Restored from supabase/migrations/20260820220000_rpc_v1_feed_bootstrap.sql
