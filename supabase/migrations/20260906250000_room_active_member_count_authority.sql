-- Authoritative active Trade Room membership visibility + bootstrap count for all members.
-- Active member = room_members row with left_at IS NULL (web loadMemberStats active count).

-- =============================================================================
-- RLS: active members may read peer active memberships (Members list parity)
-- =============================================================================

drop policy if exists "room_members_select_active_peers" on public.room_members;
create policy "room_members_select_active_peers"
...
comment on policy "room_members_select_active_peers" on public.room_members is
  'Active room members can list other active members. Banned/left users excluded.';

-- =============================================================================
-- Bootstrap: active_member_count for every active member (not owner-only)
-- =============================================================================
    ...
    'data', jsonb_build_object(
      ...
      'member_stats', case
        when v_is_owner then jsonb_build_object(
          ...
          'active_members', coalesce(v_active_members, v_active_member_count, 0),
          'left_members', greatest(
            coalesce(v_total_members, 0) - coalesce(v_active_members, v_active_member_count, 0),
            0
          )
        )
        else null
...
comment on function public.rpc_v1_room_bootstrap is
  'Phase F: Trade Room bootstrap — includes active_member_count for all active members.';
