-- Getting Started: derive daily check-in task completion from trader_daily_check_ins.
...
declare
  ...
  v_has_completed_daily_check_in boolean := false;
...
begin
  ...
  select exists (
    ...
    where ci.user_id = v_uid
      and ci.sleep_hours is not null
      and ci.sleep_hours >= 0
      and ci.sleep_hours <= 24
      and ci.sleep_quality between 1 and 5
      and ci.morning_rating between 1 and 5
      and ci.stress_level between 1 and 5
      and ci.energy_level between 1 and 5
      and ci.focus_level between 1 and 5
  )
  into v_has_completed_daily_check_in;
  ...
  return jsonb_build_object(
    ...
    'has_completed_daily_check_in', v_has_completed_daily_check_in
  );
end;
$$;
