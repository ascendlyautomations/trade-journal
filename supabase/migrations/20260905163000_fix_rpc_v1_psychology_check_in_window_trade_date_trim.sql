-- Fix rpc_v1_psychology_check_in_window: trades.trade_date is `date`, not text.
-- trim()/btrim() on date fails with PostgreSQL 42883 (function btrim(date) does not exist).
...
begin
  ...
  trade_dates as (
    select distinct coalesce(st.trade_date, st.date) as d
    from scoped_trades st
    where coalesce(st.trade_date, st.date) is not null
  )
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
