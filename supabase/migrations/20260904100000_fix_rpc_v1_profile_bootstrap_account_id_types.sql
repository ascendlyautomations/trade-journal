-- Fix rpc_v1_profile_bootstrap: trades.account_id (text) vs accounts.id (uuid).
-- Root cause of web BackendV2RpcError: operator does not exist: text = uuid
-- in the public_account_modes EXISTS subquery (added 20260903140000).
...
comment on function public.rpc_v1_profile_bootstrap(text, text, integer, text) is
  'Bounded Profile header + active-tab first page (read-only). Includes authoritative public_account_modes for stats filtering.';
