-- Profile bootstrap: align payout_total with web Profile overview.
--
-- Web source: sumPayoutAchievementTotals(achievements) in lib/achievementTypes.ts
--   - Sum achievements.value_numeric for payout achievement types
--   - Types: prop_firm_payout, live_trading_payout, legacy payout, or any type containing "payout"
--   - Owner profile: all own achievements (public + private) via fetchOwnAchievements
--   - Visitor profile: public achievements only via fetchVisibleProfileAchievements
--   - Payout header stat is shown even when trade stats are locked (can_view_trades = false)
--
-- Fixes native divergence:
--   1. Remove hard is_public filter (RLS + owner vs visitor semantics)
--   2. Match web payout type filter (LIKE '%payout%')
--   3. Always return payout_total in public_stats (including locked private profiles)
...
begin
  ...
  -- Web overviewPayoutTotal: sum payout achievements visible to the viewer (RLS-scoped).
