-- Feed bootstrap: exclude content from blocked users (bidirectional via users_have_active_block).
-- App Store Guideline 1.2 — blocked authors must not appear in Following or Global feed.

create or replace function public.get_active_block_peer_ids()
returns uuid[]
language sql
stable
security definer
...
as $$
  select coalesce(array_agg(distinct peer_id), '{}'::uuid[])
  from (
    select ub.blocked_id as peer_id
    from public.user_blocks ub
    where ub.blocker_id = auth.uid()
    union
    select ub.blocker_id as peer_id
    from public.user_blocks ub
...
comment on function public.get_active_block_peer_ids() is
  'All profile ids in an active block relationship with auth.uid() (either direction).';

revoke all on function public.get_active_block_peer_ids() from public;
grant execute on function public.get_active_block_peer_ids() to authenticated;
...
comment on function public.rpc_v1_feed_bootstrap(text, text, integer, text) is
  'Backend V2 Feed bootstrap — excludes blocked authors (bidirectional user_blocks).';
