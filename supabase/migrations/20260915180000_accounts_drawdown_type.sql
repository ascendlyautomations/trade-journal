-- Prop firm max drawdown methodology (display + firm-specific rules; not inferred client-side).

alter table public.accounts
  add column if not exists drawdown_type text;

alter table public.accounts
  drop constraint if exists accounts_drawdown_type_check;

alter table public.accounts
  add constraint accounts_drawdown_type_check
  check (
    drawdown_type is null
    or drawdown_type in ('static', 'intraday_trailing', 'end_of_day_trailing')
  );

comment on column public.accounts.drawdown_type is
  'Prop firm max drawdown methodology: static, intraday_trailing, or end_of_day_trailing.';

-- Prop firm bootstrap: expose drawdown_type on account rows.
...
begin
  ...
  select
    ...
    coalesce(array_agg(a.id::text), '{}'::text[])
  into v_accounts, v_account_ids, v_account_ids_text
  ...
  if coalesce(array_length(v_account_ids, 1), 0) > 0 then
    ...
    where t.user_id = v_uid
      ...
      and t.account_id = any (v_account_ids_text);
  end if;
