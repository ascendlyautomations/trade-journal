-- Backend V2 Phase 3: Dashboard bootstrap (single JSON for Dashboard-owned data).
-- SECURITY INVOKER — relies on existing RLS for accounts/trades/payouts.
-- Does not alter tables. Composes reads only.
-- Does NOT include session-owned fields (viewer, badges, following, prefs, profile).

-- Account identity in Backend V2 is UUID (accounts.id).
-- trades.account_id remains legacy text (stores account UUID strings); compare via
-- p_account_id::text only at that boundary — do not cast trades.account_id to uuid.
...
comment on function public.rpc_v1_dashboard_bootstrap(uuid, integer) is
  'Backend V2 Dashboard bootstrap — accounts + trade window + metrics/equity. Session fields excluded.';
