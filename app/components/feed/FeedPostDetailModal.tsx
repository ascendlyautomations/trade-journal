...
export default function FeedPostDetailModal({
  ...
  const scrollCommentsIntoView = useCallback(() => {
    requestAnimationFrame(() => {
      ...
      const input = commentsSectionRef.current?.querySelector("input")
  ...
  return (
    ...
    >
      <div
        className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-[#0f172a] p-4"
        onClick={stopPropagation}
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-2 top-2 z-10 text-xl text-white"
          aria-label="Close"
        >
          ✕
        </button>
