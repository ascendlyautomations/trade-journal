...
import NativeIosPlatformModal from "./native/NativeIosPlatformModal"
 ...
 * Web only — native uses fullscreen PlatformModal chrome.
 */
...
function buildDetailBody({
  ...
  | "comments"
>) {
  if (layout === "split" && splitPanel != null) {
    return (
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      {details ? <div className="shrink-0 overflow-hidden">{details}</div> : null}
      {comments ? (
...
function WebDetailModalShell({
  ...
  const dialogWidthClass =
    layout === "split" ? "max-w-2xl md:max-w-[760px]" : "max-w-2xl"
...
/**
 * Content-heavy popup presentation.
 * Native iOS: fullscreen (no navbar offset). Web: existing DetailModalShell card.
 */
export default function PlatformModal(props: PlatformModalProps) {
  const { isNativeIos } = usePlatformPresentation()
  const body = buildDetailBody(props)
  ...
  if (isNativeIos) {
    return (
      <NativeIosPlatformModal
        ...
        title={props.title}
        zIndexClass={props.zIndexClass ?? DETAIL_MODAL_Z_INDEX_CLASS}
      >
        {body}
      </NativeIosPlatformModal>
    )
  ...
  return <WebDetailModalShell {...props} />
}
