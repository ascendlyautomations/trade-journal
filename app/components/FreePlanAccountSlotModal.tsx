...
export default function FreePlanAccountSlotModal({
  ...
  const canConfirm = selectedIds.length === FREE_PLAN_ACCOUNT_LIMIT && !saving
      ...
      footer={
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-gray-400">
            Selected {selectedIds.length} of {FREE_PLAN_ACCOUNT_LIMIT}
          </p>
          <button
            type="button"
            disabled={!canConfirm}
    ...
    >
      <div className="space-y-4">
        <div className="space-y-2 text-sm leading-relaxed text-gray-300">
          <p>
            The Free plan includes up to {FREE_PLAN_ACCOUNT_LIMIT} active trading
            accounts.
          </p>
          <p>Choose the accounts you want to continue trading with.</p>
          <p>
            Your other accounts and ALL trade history will remain available in
            read-only mode.
          </p>
        </div>
