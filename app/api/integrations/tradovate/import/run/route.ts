...
export async function POST(req: Request) {
  ...
  const allTargets = await listLinkedTradovateImportTargets(integrationDb, user.id)
  if (allTargets.length === 0) {
    return Response.json({ error: "No linked Tradovate accounts." }, { status: 400 })
  }
  ...
  const batch = await runTradovateManualImportBatch(integrationDb, user.id, targets)
