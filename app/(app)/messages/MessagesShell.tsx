...
export default function MessagesShell({ children }: { children: ReactNode }) {
  ...
  const onThread = isDmConversationPath(pathname)
  const activeSegment = onThread
    ? decodeURIComponent(pathname.split("/").filter(Boolean)[1] ?? "")
    : ""
  const selectedConversationId =
    conversations.find((conversation) => {
      if (!activeSegment) return false
      if (conversation.id === activeSegment) return true
      const username = normalizeProfileUsername(conversation.username ?? "")
      return (
        username.length > 0 &&
        username === normalizeProfileUsername(activeSegment)
      )
    })?.id ?? null
  ...
  return (
    <>
      ...
      <div
        className={
          onThread
            ? "hidden w-full min-w-0 xl:flex xl:h-full xl:w-[380px] xl:shrink-0"
            : "flex w-full min-w-0 xl:h-full xl:w-[380px] xl:shrink-0"
        }
      >
      <div
      ...
      <div
        className={
          onThread
            ? "flex min-h-0 w-full min-w-0 flex-1 flex-col xl:h-full"
            : "hidden min-h-0 min-w-0 flex-1 flex-col xl:flex xl:h-full"
        }
      >
        {children}
      </div>
