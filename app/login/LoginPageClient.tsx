...
export default function LoginPageClient({
  ...
  return (
  <NativeIosLoginShell initialNativeIos={initialNativeIos}>
  {(nativeIos) => (
  <>
    ...
    {/* Web only — native login is the root auth screen (no back). */}
    {!nativeIos ? (
    <button
      type="button"
      onClick={handleBack}
    ...
    <div
      ...
      style={
        nativeIos
          ? {
              // Raise the block ~40px; safe-area clears Dynamic Island / notch
              // now that the WebView paints edge-to-edge under the status bar.
              paddingTop: "max(0.75rem, calc(var(--safe-area-top) + 0.25rem))",
              paddingBottom:
