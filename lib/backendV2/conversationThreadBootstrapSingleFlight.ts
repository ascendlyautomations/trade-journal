...
const FLIGHT_SYMBOL = Symbol.for("tradetraxs.conversationThread.flights")
...
export function getConversationThreadFlight<T>(key: string): Promise<T> | null {
  return (store().byKey.get(key)?.promise as Promise<T> | undefined) ?? null
}

export function beginConversationThreadFlight<T>(
  key: string,
  userId: string,
  start: () => Promise<T>
): Promise<T> {
...
export function clearConversationThreadFlights(userId?: string | null): void {
  const s = store()
  if (!userId) {
    s.byKey.clear()
    return
...
export function __resetConversationThreadFlightsForTests(): void {
  store().byKey.clear()
}
