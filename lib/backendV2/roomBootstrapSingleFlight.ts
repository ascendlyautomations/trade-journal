...
const FLIGHT_SYMBOL = Symbol.for("tradetraxs.roomBootstrap.flights")
...
export function getRoomBootstrapFlight<T>(key: string): Promise<T> | null {
  return (store().byKey.get(key)?.promise as Promise<T> | undefined) ?? null
}

export function beginRoomBootstrapFlight<T>(
  key: string,
  userId: string,
  start: () => Promise<T>
): Promise<T> {
...
export function clearRoomBootstrapFlights(userId?: string | null): void {
  const s = store()
  if (!userId) {
    s.byKey.clear()
    return
...
export function __resetRoomBootstrapFlightsForTests(): void {
  store().byKey.clear()
}
