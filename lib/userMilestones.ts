...
const SYSTEM_MILESTONE_IDS = new Set<SystemMilestoneId>([
  "first_trade",
  "trades_10",
  "trades_100",
  "trades_500",
...
function isSystemMilestoneId(id: string): id is SystemMilestoneId {
  return SYSTEM_MILESTONE_IDS.has(id as SystemMilestoneId)
}
...
export function readStickyMilestoneUnlocks(userId: string): Set<SystemMilestoneId> {
  ...
  try {
    ...
    return new Set(
      parsed.filter(
        (id): id is SystemMilestoneId =>
          typeof id === "string" && isSystemMilestoneId(id)
      )
    )
  } catch {
    return new Set()
