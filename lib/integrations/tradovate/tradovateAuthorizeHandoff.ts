...
export function createTradovateAuthorizeHandoffValue(userId: string): string {
  const exp = Math.floor(Date.now() / 1000) + HANDOFF_TTL_SECONDS
  const payloadB64 = Buffer.from(JSON.stringify({ uid: userId, exp }), "utf8").toString(
    "base64url"
  )
...
export function verifyTradovateAuthorizeHandoffValue(value: string): string | null {
