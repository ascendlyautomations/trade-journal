...
/// Shared wire → ``ProfileStatisticsMetrics.Result`` mapping for V1 and V2 Profile analytics bootstraps.
nonisolated enum ProfileStatisticsBootstrapModeMapping {
    struct ModeWire: Codable, Sendable {
    ...
    struct SessionRowWire: Codable, Sendable {
        var label: String
        var count: Int
        var pct: Double
    }

    struct EquityPointWire: Codable, Sendable {
        var index: Int
        var equity: PostgresFlexibleDouble?
        var date: String?
    }

    nonisolated static func mapModes(
        _ modes: [String: JSONValue]
    ) -> [ProfileStatisticsMetrics.Mode: ProfileStatisticsMetrics.Result] {
    ...
    static func result(from wire: ModeWire) -> ProfileStatisticsMetrics.Result {
        mapResult(wire)
    }
